...
describe('WalletManager', () => {
  ...
  beforeEach(() => {
    chain1 = createMockChain({
      chainName: 'chain-1',
      chainId: '1',
      rpcEndpoint: [{ address: 'http://localhost:26657' }],
      chainType: 'cosmos',
    });
    chain2 = createMockChain({
      chainName: 'chain-2',
      chainId: '2',
      rpcEndpoint: [{ address: 'http://localhost:26658' }],
      chainType: 'cosmos',
    });
    ...
    endpointOptions = {
      endpoints: { 'chain-1': { rpc: ['http://localhost:26657'] } },
    };
  ...
  it('should have right signerOptions and endpointOptions for each chain', () => {
    expect(walletManager.signerOptionMap[chain1.chainName]).toEqual(
      signerOptions.signing?.(chain1.chainName)
    );
    expect(walletManager.signerOptionMap[chain2.chainName]).toEqual(
      signerOptions.signing?.(chain2.chainName)
    );
    expect(walletManager.endpointOptionsMap[chain1.chainName]).toEqual(
      endpointOptions.endpoints?.[chain1.chainName]
    );
    expect(walletManager.endpointOptionsMap[chain2.chainName]).toEqual(
      undefined
    );
    expect(walletManager.preferredSignTypeMap[chain1.chainName]).toEqual(
      'amino'
    );
    expect(walletManager.preferredSignTypeMap[chain2.chainName]).toEqual(
      'direct'
    );
  });
  ...
  it('should add new chains', async () => {
    const newChain = createMockChain({
      chainName: 'chainToAdd',
      chainId: '3',
      rpcEndpoint: [{ address: 'http://localhost:26659' }],
      chainType: 'cosmos',
    });
    const newAssetList = createMockAssetList({
      chainName: 'chainToAdd',
      assets: [],
    });
    ...
    expect(walletManager.signerOptionMap[newChain.chainName]).toEqual(
      signerOptions.signing?.(newChain.chainName)
    );
  });
  ...
  it('should connect to a wallet', async () => {
    ...
    await walletManager.connect(
      wallet1.info?.name as string,
      chain1.chainName as string
    );
    expect(wallets[0].connect).toHaveBeenCalledWith(chain1.chainId);
  });

  it('should throw error if wallet does not exist', async () => {
    await expect(walletManager.connect('wallet-3', 'chain-1')).rejects.toThrow(
      WalletNotExist
    );
  });

  it('should throw error if chain does not exist', async () => {
    await expect(walletManager.connect('wallet-1', 'chain-3')).rejects.toThrow(
      ChainNameNotExist
    );
  });
  ...
  it('should get right account from a wallet', () => {
    ...
    wallet1.getAccount = jest
      .fn()
      .mockReturnValue(Promise.resolve(mockAccount1));
    walletManager.getAccount(wallet1.info?.name as string, chain1.chainName);
    expect(wallets[0].getAccount(chain1.chainId as string)).resolves.toBe(
      mockAccount1
    );
  });
  ...
  it('should throw error if no valid RPC endpoint found', async () => {
    ...
    await expect(
      walletManager.getRpcEndpoint('wallet-1', 'chain-1')
    ).rejects.toThrow(NoValidRpcEndpointFound);
  });
  ...
  it('should get signer options', () => {
    ...
    expect(options.signerOptions?.prefix).toBe(
      signerOptions.signing?.(chain1.chainName as string)?.signerOptions?.prefix
    );
  });
  ...
  it('should get signing client', async () => {
    ...
    const mockSignerOption = signerOptions.signing?.(
      chain1.chainName as string
    );
    const mockRpcEndpoint = 'http://localhost:26657';

    walletManager.getOfflineSigner = jest
      .fn()
      .mockReturnValue(Promise.resolve(mockAminoOfflineSigner));
    walletManager.getSignerOptions = jest
      .fn()
      .mockReturnValue(mockSignerOption);
    walletManager.getRpcEndpoint = jest
      .fn()
      .mockReturnValue(Promise.resolve(mockRpcEndpoint));

    await walletManager.getSigningClient(
      wallet1.info?.name as string,
      chain1.chainName as string
    );

    expect(SigningClient.connectWithSigner).toHaveBeenCalledWith(
      mockRpcEndpoint,
      mockAminoOfflineSigner,
      mockSignerOption
    );
  });
  ...
  it('should get download link', () => {
    ...
    walletManager.getEnv = jest
      .fn()
      .mockReturnValue({ browser: 'chrome', device: 'desktop' });
  ...
  it('addChains should update right signerOptions for new chains', async () => {
    ...
    expect(walletManager.endpointOptionsMap[newChain.chainName]).toEqual({
      rpc: ['http://localhost:26679'],
    });
  ...
  it('addChains should update endpoint from chain rpc list', async () => {
    (getValidRpcEndpoint as jest.Mock).mockReturnValue(
    ...
    const newChain = createMockChain({
      chainName: 'chainToAdd',
      chainId: '3',
      rpcEndpoint: [{ address: 'https://rpc.example.com' }],
      chainType: 'cosmos',
    });
    const newAssetList = createMockAssetList({
      chainName: 'chainToAdd',
